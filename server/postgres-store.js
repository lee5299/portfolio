...
export class PostgresStore {
  ...
  async #hydrateAccount(row) {
    ...
    const [passkeys, privateItems] = await Promise.all([
      this.sql`select * from portfolio_passkey.passkeys where account_id = ${row.id} order by created_at`,
      this.sql`select * from portfolio_passkey.private_items where account_id = ${row.id} order by created_at`,
    ]);
    return {
      id: row.id,
      alias: row.alias,
  ...
  async getAccountById(accountId) {
    const [row] = await this.sql`select * from portfolio_passkey.accounts where id = ${accountId}`;
    return this.#hydrateAccount(row);
  }

  async getAccountByAlias(alias) {
    const [row] = await this.sql`select * from portfolio_passkey.accounts where alias = ${alias}`;
    return this.#hydrateAccount(row);
  }
  ...
  async getAccountByCredentialId(credentialId) {
    ...
    const [row] = await this.sql`
      ...
      from portfolio_passkey.passkeys p
      join portfolio_passkey.accounts a on a.id = p.account_id
  ...
  async addPasskey(accountId, passkey, { requireEmpty = false } = {}) {
    try {
      return await this.sql.begin(async (transaction) => {
        ...
        await transaction`
          insert into portfolio_passkey.passkeys
  ...
  async updatePasskeyUsage(accountId, credentialId, counter, lastUsedAt, expectedCounter) {
    const rows = await this.sql`
      update portfolio_passkey.passkeys
  ...
  async deletePasskey(accountId, credentialId) {
    return this.sql.begin(async (transaction) => {
      ...
      const [{ count }] = await transaction`
        select count(*)::int as count from portfolio_passkey.passkeys where account_id = ${accountId}
      `;
      if (count <= 1) return 'last-passkey';
      await transaction`
        delete from portfolio_passkey.passkeys where account_id = ${accountId} and credential_id = ${credentialId}
      `;
      return 'deleted';
    });
  }
