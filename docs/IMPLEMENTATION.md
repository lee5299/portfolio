...
자동 시험과 Production 실기 결과는 [TEST_EVIDENCE.md](TEST_EVIDENCE.md)에 있다. 실제 owner 패스키 두 개와 peer 패스키 등록, 하나 삭제 후 복구, 삭제 키 거절, 마지막 삭제 차단, 양방향 계정 격리와 일회성 ceremony 재사용 거절을 확인했다. 공개키 원문, credential ID, assertion, 쿠키와 등록 코드는 증거에 보관하지 않았다.

## 5. AI와 나

- **AI에게 맡긴 일:** 요구사항 구조화, 서버·클라이언트 구현, 자동 시험, 문서와 검증 절차 작성.
- **사용자가 직접 판단한 일:** Vercel + Supabase 사용, 마지막 패스키 삭제 차단, 실제 인증 자료의 Git 제외.
- **따르지 않은 AI 제안:** SQL Editor에 평문 DB 비밀번호를 입력하는 초기 방안은 로그 잔류 위험 때문에 사용하지 않고 로컬 SCRAM 검증값 방식으로 바꿨다.
...
- Planner와 포트폴리오가 Supabase 인스턴스의 연결 수·용량·장애 범위를 공유한다. 현재 기능은 격리 스키마와 최소 권한 역할을 사용하지만 과제 종료 뒤 최종 통합 구조에 맞춰 임시 연결을 정리해야 한다.
